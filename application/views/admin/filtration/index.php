        ...
        <div class="col-md-12">
            <div class="box-showing">
                <div class="tab-header text-center">
                    <h4>Réglages plages horaires</h4>
                </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" id="flexSwitchCheckDefault" <?= ($info[0]->$filtration_status == 1) ? 'checked' : '' ?> onclick="set_filtration_time(1, '<?= $time_set ?>', '<?= $i ?>');">
                                    </div>
                                </td>
                                <td>
                                    <div class="form-check form-switch">
                                        <input class="form-check-input ozone-choc" type="checkbox" id="flexSwitchCheckDefault" <?= ($info[0]->$filtration_status == 2) ? 'checked' : '' ?> onclick="set_filtration_time(2, '<?= $time_set ?>', '<?= $i ?>');">
                                    </div>
                                </td>
                            </tr>
                        <?php } ?>
...
<script>
    function set_filtration_time(status, time_set, row) {
        ...
        $.post("<?= base_url('admin/filtration/set_filration_time') ?>", {
            ...
            if ($('#online_setting').val() == 1) {
                connect_path_server(status, time_set, row, ft_zone, ft_status);
            } else {
    ...
    function connect_path_server(status, time_set, row, ft_zone, ft_status) {
        var path_server = $('#path_server').val() + 'api/Receive_setting/set_filration_time';
        $.post(path_server, {
            status: status,
            time_set: time_set,
            ft_zone: ft_zone,
